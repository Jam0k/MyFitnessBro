...
def createMeal():
    try:
        ...
        return render_template(
            "nutrition/meals-and-food/meal/create-meal.html", food_items=food_items_data
        )

    except Exception as e:
        # Handle exceptions and possibly return an error message
        return render_template(
            "nutrition/meals-and-food/meal/create-meal.html", error=str(e)
        )
...
def createNewMeal():
    try:
        ...
        for item in food_items_data:
            ...
            serving_count = float(
                item["serving_count"]
            )  # Convert serving count to float

            meal_food_item = MealFoodItem(
                meal_id=new_meal.id, food_item_id=food_id, serving_count=serving_count
            )
...
def browseMeal():
    try:
        ...
        for meal in meals:
            ...
            food_items_data = (
                db.session.query(FoodItem.name, MealFoodItem.serving_count)
                .join(MealFoodItem, FoodItem.id == MealFoodItem.food_item_id)
                .filter(MealFoodItem.meal_id == meal.id)
                .all()
            )

            # Format the food items and serving counts into a readable string
            food_items_str = ", ".join(
                [f"{fi.name} ({fi.serving_count} servings)" for fi in food_items_data]
            )

            meals_data.append(
                {"id": meal.id, "name": meal.name, "food_items": food_items_str}
            )

        return render_template(
            "nutrition/meals-and-food/meal/browse-meal.html", meals=meals_data
        )
    except Exception as e:
        return render_template(
            "nutrition/meals-and-food/meal/browse-meal.html", error=str(e)
        )
...
def getMeal(id):
    try:
        meal = Meal.query.get_or_404(id)
        food_items = (
            MealFoodItem.query.filter_by(meal_id=id)
        ...
        return jsonify(
            {"id": meal.id, "name": meal.name, "food_items": food_items_data}
        )
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def logMeal():
    try:
        ...
        new_meal_log = FoodMealLog(
            meal_id=meal_id, meal_type=meal_type, log_date=log_date
        )
    ...
    try:
        ...
        for meal in meals:
            ...
            meal_dict["food_items"] = [
                {"name": fi.name, "serving_count": str(fi.serving_count)}
