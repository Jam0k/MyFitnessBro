...
# Create new exercise
@fitness_blueprint.route("/create-exercise", methods=["GET", "POST"])
def create_exercise():
...
@fitness_blueprint.route("/get-exercises")
def get_exercises():
    """
    ...
    if exercise:
        if ExerciseLog.query.filter_by(exercise_id=exercise.id).first():
            return (
                jsonify(
                    {
                        "error": "Cannot delete exercise because it is referenced in exercise logs"
                    }
                ),
                400,
            )
...
# Get Exercise
@fitness_blueprint.route("get-exercise/<int:exercise_id>")
    ...
    if request.method == "POST":
        try:
            ...
            new_log = ExerciseLog(
                ...
                weight=data.get(
                    "weight", 0
                ),  # Defaulting to 0 if weight is not provided
                notes=data.get(
                    "notes", ""
                ),  # Defaulting to an empty string if notes are not provided
...
def tracking():
    ...
    if request.method == "POST":
        ...
        if selected_date:
            ...
            exercise_logs = (
                ExerciseLog.query.filter(ExerciseLog.log_date == selected_date)
                .join(ExerciseLog.exercise)
                .all()
            )

            # Query cardio logs for the selected date
            cardio_logs = CardioLog.query.filter(CardioLog.date == selected_date).all()
...
def get_cardio_logs():
    selected_date_str = request.form.get("date")
    selected_date = (
        datetime.strptime(selected_date_str, "%Y-%m-%d") if selected_date_str else None
    )
