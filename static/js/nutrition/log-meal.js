$(document).ready(function () {
  ...
  // Set default date to today for the meal log
  var today = new Date().toISOString().split("T")[0];
  $("#logDateMeal").val(today);
        ...
        columns: [
          { data: "name" },
          {
            ...
            render: function (data) {
              return data
                .map((item) => `${item.name} (${item.serving_count} servings)`)
                .join(", ");
            },
          },
          {
  ...
  $("#logMealForm").on("submit", function (e) {
    ...
    var logDateMeal = $("#logDateMeal").val();  // Correctly getting the value from the date input

    var logData = {
        meal_id: selectedMealId,
        meal_type: mealMealType,
        log_date: logDateMeal,  // Correctly referencing the date value
    };
